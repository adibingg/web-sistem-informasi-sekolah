 <div class="row no-gutter fullwidth"><!-- row -->
    <div class="col-lg-12 clearfix"><!-- featured posts slider -->
        <div id="carousel-featured" class="carousel slide" data-interval="4000" data-ride="carousel">
            <div class="carousel-inner"><!-- Wrapper for slides -->          
                <div class="item active">
                    <img src="<?php echo base_url().'assets/'?>front/img/6.png" alt="Image slide 3" />
                    <div class="k-carousel-caption pos-1-3-right scheme-dark">
                        <div class="caption-content">
                            <h3 class="caption-title">SMK MA'ARIF 1 KALIREJO</h3>
                            <b style="text-align: justify;">Bertaqwa, Berpengalaman & Mempersiapkan diri memasuki Dunia Usaha atau Kerja dan Perguruan Tinggi</b>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <img src="<?php echo base_url().'assets/'?>front/img/6.png" alt="Image slide 3" />
                    <div class="k-carousel-caption pos-1-3-right scheme-dark">
                        <div class="caption-content">
                            <h3 class="caption-title">Program Keahlian</h3>
                        <div>
                    </div>
                </div>                                                  
            </div>
            <a class="left carousel-control" href="#carousel-featured" data-slide="prev"><i class="fa fa-chevron-left"></i></a>
            <a class="right carousel-control" href="#carousel-featured" data-slide="next"><i class="fa fa-chevron-right"></i></a>
        </div>
    </div>
</div>         
